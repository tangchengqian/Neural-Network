...
X = xlsread('分解后的周期数据集测试.xlsx','A2:F1347');
...
alpha = 50;       % moderate bandwidth constraint：适度的带宽约束/惩罚因子
...
% ===== 新增代码：将分解结果输出到Excel文件 =====
% 创建表格数据
resultTable = table(t', X(:,end), 'VariableNames', {'Time_hour', 'Original_Signal'});

% 添加各IMF分量
for i = 1:K
    resultTable.(['IMF' num2str(i)]) = u(i,:)';
end

% 输出到Excel文件
outputFilename = 'VMD_Decomposition_Results.xlsx';
writetable(resultTable, outputFilename);

disp(['VMD分解结果已保存至: ' outputFilename]);
% ===== 新增代码结束 =====
